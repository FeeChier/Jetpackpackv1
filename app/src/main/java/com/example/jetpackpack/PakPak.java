...
public class PakPak extends BaseObject{
    private ArrayList<Bitmap> arrBms = new ArrayList<>();
    private int count, vFly, idCurrentBitmap;
    private float drop;
    ...
    public Bitmap getBm() {
        ...
        if (this.drop < 0) {
            ...
            return Bitmap.createBitmap(arrBms.get(idCurrentBitmap),0,0,arrBms.get(idCurrentBitmap).getWidth(),arrBms.get(idCurrentBitmap).getHeight(),/*A enlever car pas besoin de rotation*/matrix,true);
        } else if (drop >= 0) {
            Matrix matrix = new Matrix();
            if (drop < 70) {
